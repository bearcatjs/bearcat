...
BeanFactory.prototype.doGetModelProxy = function(modelId) {
	...
	var beanDefinition = this.getBeanDefinition(modelId);
	...
	var model = this.getBean(modelId);
	modelFilter.setModel(model);
	modelProxy.setModel(model);
	modelProxy.setModelFilter(modelFilter);
	modelProxy.setBeanDefinition(beanDefinition);
	modelProxy.setModelDefinition(modelDefinition);
...
BeanFactory.prototype.registerBean = function(beanName, metaObject) {
	...
	if (!ValidatorUtil.metaValidator(metaObject)) {
		logger.error("registerBean %j metaObject %j validate error", beanName, metaObject);
		return;
	}
	...
	if (mid) {
		...
		this.registerModel(mid, metaObject);
	}
 ...
 * @api public
 */
BeanFactory.prototype.registerModel = function(modelId, metaObject) {
	...
	// modelDefinition.setNormalFields(resolvedAttributes['normal']);
	// modelDefinition.setPrimaryFields(resolvedAttributes['primary']);
	modelDefinition.setFields(resolvedAttributes);

	this.models[modelId] = modelDefinition;
}
